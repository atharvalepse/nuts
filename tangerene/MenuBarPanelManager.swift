...
extension Notification.Name {
    static let tangereneDismissPanel = Notification.Name("tangereneDismissPanel")
}
...
final class MenuBarPanelManager: NSObject {
    ...
    private func createStatusItem() {
        ...
        button.image = makeTangereneMenuBarIcon()
        button.image?.isTemplate = true
        button.action = #selector(statusItemClicked)
        button.target = self
    }

    /// Draws the tangerene triangle as a menu bar icon. Uses the same shape
    /// and rotation as the in-app cursor so the menu bar icon matches.
    private func makeTangereneMenuBarIcon() -> NSImage {
