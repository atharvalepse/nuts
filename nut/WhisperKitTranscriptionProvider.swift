...
private final class WhisperKitTranscriptionSession: BuddyStreamingTranscriptionSession {
    ...
    private let stateQueue = DispatchQueue(label: "com.nut.whisperkit.transcription")
    ...
    deinit {
        // Do NOT call cancel() here: it dispatches an async block that captures
        // `self`, which retains an instance that is mid-deallocation. In debug
        // builds the Swift runtime traps on that (SIGABRT during __deallocating_deinit).
        // Just cancel the in-flight task synchronously — the buffered audio is
        // released as the instance deallocates anyway.
        transcriptionTask?.cancel()
    }
}
